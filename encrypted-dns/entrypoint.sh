...
init() {
    ...
    while getopts "h?N:E:D:T:AM:" opt; do
        ...
        D) dns_upstream_address=$(echo "$OPTARG" | sed -e 's/^[ \t]*//' | tr A-Z a-z) ;;
    ...
    *) provider_name="2.dnscrypt-cert.${provider_name}" ;;
    ...
    dns_upstream_configuration=""
    if [ -n "$dns_upstream_address" ]; then
        dns_upstream_configuration="upstream_addr = \"${dns_upstream_address}\""
    fi
    ...
    sed \
        -e "s#@DNS_UPSTREAM_CONFIGURATION@#${dns_upstream_configuration}#" \
...
To configure the upstream DNS server add -D <dns server ip>:<port>
